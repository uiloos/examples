...
<script lang="ts">
  ...
  /*
    ...
    This causes Svelte to re-render and since the `WeekEvent` is no longer 
    the child of his old `DayGrid` it is removed from the DOM and re-created.

    But this causes a problem for us: if we used Svelte `on:drag`, the 
    dragging stops. When an element is removed Svelte no longer sends 
    dragging events. This makes sense since the element from Svelte 
    ...
    behind Svelte's back, and cleaning it up manually. This way the 
    element will stay in memory, due to us keeping a reference to it.
  */

  $: eventDrag(eventRef) 

  function eventDrag(e: HTMLLIElement | null = null) {
    if (eventRef) {
      eventRef.ondrag = (e: DragEvent) => {
        onDrag(e);
      };

      eventRef.ondragend = (e: DragEvent) => {
        if (eventRef) {
          eventRef.ondrag = null;
          eventRef.ondragend = null;
        }    
      };
    }
  }
  ...
  function onDrag(e: DragEvent) {
    ...
    if (
      ...
      if (dragData.mode === "event") {
        const duration =
          dragData.dragEndTime - dragData.dragStartTime;
    ...
    */
    if (event.spansMultipleDays) {
      if (dateGallery.isSameDay(event.startDate, date)) {
        ...
        // No time drag indicator as it is on another day
