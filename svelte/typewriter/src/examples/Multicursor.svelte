<script lang="ts">
  ...
  import type { TypewriterPosition } from '@uiloos/core';
  ...
  let typewriter = createTypewriterStore<Data>({
    repeat: true,
    repeatDelay: 10000,
    cursors: [
      {
...
<div id="multicursor-typewriter">
  {#each $typewriter as position}
    {#each position.cursors.toReversed() as cursor}
      <span
        class="cursor"
        class:blink={cursor.isBlinking}
        style="--cursor-color: {cursor.data?.color}"
      ></span>
      <span class="info" style="--cursor-color: {cursor.data?.color}"
        >{cursor.data?.name}</span
      >
    {/each}<span
      class="letter"
      style="--background-color: {getBackgroundColorForPosition(position)}"
