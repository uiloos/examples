...
const messageEl = document.getElementById("dateRangePickerMessage");

// Do something on submit
document.getElementById("vacationForm").onsubmit = (e) => {
  e.preventDefault();

  const formData = new FormData(e.target);

  messageEl.textContent = `Your vacation starts on: ${formData.get("startDate")} and ends on ${formData.get("endDate")}`;
};

// Clear message first when submit button is clicked
// prevents old message from showing when form became
// invalid later.
document.querySelector("#vacationForm button[type='submit']").onclick = () => {
