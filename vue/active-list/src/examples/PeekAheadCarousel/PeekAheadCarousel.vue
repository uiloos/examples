  ...
  <div>
    ...
    >
      <li
        ...
        :id="`peek-ahead-carousel-slide-${content.value.id}`"
        class="slide"
      >
        <img
          width="1920"
...
watchEffect(() => {
  ...
  if (carousel.value.activeContents.length === 3) {
    const previousPreviousSlide = carousel.value.activeContents[0];
    ...
    window.setTimeout(() => {
      ...
      window.setTimeout(() => {
        if (carouselElement.value) {
          carouselElement.value.scrollLeft = carouselElement.value.clientWidth;
        }
      }, 1);
    }, 1000);
  }
